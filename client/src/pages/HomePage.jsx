...
const slides = [
  { title: "Premium Orthopedic Mattresses ", text: "Dr. Ortho King offers supportive Turkish-made mattresses, pillows, and bedroom essentials for healthier sleep at home.", image: "/assets/hero5.webp", mobileImage: "/assets/mobile%201.jpg" },
  { title: "Comfort Built for Better Sleep", text: "Choose the feel and finish that make your bedroom feel restorative, calm, and tailored to you.", image: "/assets/hero1.jpeg", mobileImage: "/assets/mobile%202.jpg" },
  { title: "Support for a Happier Back", text: "Experience exceptional comfort and pressure relief with expertly crafted sleep solutions for every night.", image: "/assets/drs.webp", mobileImage: "/assets/mobile%203.jpg" },
];
const categories = [
...
export function HomePage({ settings }) {
  ...
  return <>
    <Seo
      title="Premium Orthopedic Mattresses in Uganda"
      description="Shop premium orthopedic mattresses, bedroom furniture, and sleep essentials in Uganda from Dr. Ortho King."
      path="/"
    />
    <div className="euro-home">
      <section className="euro-hero"><picture key={slide}><source media="(max-width: 800px)" srcSet={slides[slide].mobileImage} /><img src={slides[slide].image} alt="Comfortable bedroom" /></picture><div className="euro-hero-shade" /><div className="euro-hero-copy"><div className="hero-slide-panel" key={slide}><h1>{slides[slide].title}</h1><p>{slides[slide].text}</p><div><Link to="/shop" className="euro-button gold">Shop Now</Link></div></div></div><div className="slide-dots">{slides.map((item, index) => <button key={item.title} className={slide === index ? "selected" : ""} onClick={() => setSlide(index)} aria-label={`Slide ${index + 1}`} />)}</div></section>
